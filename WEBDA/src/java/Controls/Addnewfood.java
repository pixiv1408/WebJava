 ...
 * @author DELL
 */
@WebServlet(name = "Addnewfood", urlPatterns = {"/Addnewfood"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (login.getAccount() == null) {
                ...
                String fdescribe = request.getParameter("fdescribe");
                int fcate = Integer.parseInt(request.getParameter("fcate"));
                DAO dao = new DAO();
                if (fname == "" || fprice == 0 || fimg == "" || fdescribe == "") {
                    String mess = "không thể điền thiếu thông tin !";
                    request.setAttribute("Mess", mess);
                    request.getRequestDispatcher("ManagerProduct.jsp").forward(request, response);
                } else {

                    dao.faddnew(fname, fprice, fcate, fimg, fdescribe, 1);
